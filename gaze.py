...
#def fixation_group():
 #   global fixation_points
  #  i = len(fixation_points)
   # fixation_points = list(fixation_points)
#    for k in range(0, i-2):
 #       # Calculating the distance b/w two fixation points
  #      distance = math.sqrt((fixation_points[i-1][0] - fixation_points[i-2][0])**2 +
   #                          (fixation_points[i - 1][1] - fixation_points[i - 2][1]) ** 2)
##
  #      if distance > 0.5:
   #         del fixation_points[i-1]
    #    else:
     #       fix_group = fix_group + fixation_points[i-1]
  #  print(fix_group)
